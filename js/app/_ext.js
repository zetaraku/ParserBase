define(['exports'], function (exports) {
	...
	function equals(_this, _that) {
		if (typeof _this[overrides.equals] === 'function') {
			...
			if (!(_that instanceof Map) || _this.size !== _that.size) return false;
			for (let k of _this.keys()) {
				if (!_that.has(k)) return false;
				let vThis = _this.get(k),
				    vThat = _that.get(k);
				if (!equals(vThis, vThat)) return false;
			}
			return true;
		} else if (_this instanceof Set) {
			if (_this === _that) return true;
			if (!(_that instanceof Set) || _this.size !== _that.size) return false;
			for (let e of _this) if (!_that.has(e)) return false;
			return true;
		} else if (_this instanceof Array) {
			if (_this === _that) return true;
			if (!(_that instanceof Array) || _this.length !== _that.length) return false;
